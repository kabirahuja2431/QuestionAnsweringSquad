...
from modules import RNNEncoder, SimpleSoftmaxLayer,BiDirLSTMEncoder,CoAttn

logging.basicConfig(level=logging.INFO)


class QAModel_CoAttn(object):
    ...
    def __init__(self, FLAGS, id2word, word2id, emb_matrix):
        ...
        with tf.variable_scope("QAModel_CoAttn", initializer=tf.contrib.layers.variance_scaling_initializer(factor=1.0, uniform=True)):
    ...
    def build_graph(self):
        ...
        attn_layer = CoAttn(self.keep_prob, self.FLAGS.hidden_size)
        ...
        lstmEncoder = BiDirLSTMEncoder(self.FLAGS.hidden_size,self.keep_prob)
        lstm_output = lstmEncoder.build_graph(attn_output,self.context_mask)

        # Apply fully connected layer to each blended representation
        # Note, blended_reps_final corresponds to b' in the handout
        # Note, tf.contrib.layers.fully_connected applies a ReLU non-linarity here by default
        hidden_fc = tf.contrib.layers.fully_connected(lstm_output, num_outputs=self.FLAGS.hidden_size) # blended_reps_final is shape (batch_size, context_len, hidden_size)

        # Use softmax layer to compute probability distribution for start location
        # Note this produces self.logits_start and self.probdist_start, both of which have shape (batch_size, context_len)
        with vs.variable_scope("StartDist"):
            softmax_layer_start = SimpleSoftmaxLayer()
            self.logits_start, self.probdist_start = softmax_layer_start.build_graph(hidden_fc, self.context_mask)

        # Use softmax layer to compute probability distribution for end location
        # Note this produces self.logits_end and self.probdist_end, both of which have shape (batch_size, context_len)
        with vs.variable_scope("EndDist"):
            softmax_layer_end = SimpleSoftmaxLayer()
            self.logits_end, self.probdist_end = softmax_layer_end.build_graph(hidden_fc, self.context_mask)
