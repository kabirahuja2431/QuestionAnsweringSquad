...
from vocab_chars import PAD_ID, UNK_ID,charPad_ID, charUNK_ID


class Batch(object):
    """A class to hold the information needed for a training batch"""

    def __init__(self, context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, char_context_ids, char_qn_ids, ans_span, ans_tokens, uuids=None):
        """
        Inputs:
...
def sentence_to_char_ids(sentence, char2id,char_len):
    words = split_by_whitespace(sentence)
    char_ids = []
    char_tokens = []
    for word in words:
        chars = list(word)
        if(len(chars) > char_len):
            chars = chars[:char_len]
        ids = [char2id.get(c,charUNK_ID) for c in chars]
        ids += [0]*(char_len - len(ids))
        char_ids.append(ids)
...
def refill_batches(batches, word2id,char2id, context_file, qn_file, ans_file, batch_size, context_len, question_len, char_len, discard_long):
    """
    ...
    while context_line and qn_line and ans_line: # while you haven't reached the end
        ...
        char_context_ids = sentence_to_char_ids(context_line,char2id,char_len)
        qn_tokens, qn_ids = sentence_to_token_ids(qn_line, word2id)
        char_qn_ids = sentence_to_char_ids(qn_line,char2id,char_len)
        ...
        char_contexts.append(char_context_ids)

        char_qns.append(char_qn_ids)
    ...
    for batch_start in xrange(0, len(examples), batch_size):
        ...
        char_context_ids_batch = char_contexts[batch_start:batch_start+batch_size]
        char_qn_ids_batch = char_qns[batch_start:batch_start+batch_size]
        batches.append((context_ids_batch, context_tokens_batch, qn_ids_batch, qn_tokens_batch,char_context_ids_batch,char_qn_ids_batch,ans_span_batch, ans_tokens_batch))
...
def get_batch_generator(word2id,char2id, context_path, qn_path, ans_path, batch_size, context_len, question_len, char_len, discard_long):
    """
    ...
    while True:
        if len(batches) == 0: # add more batches
            refill_batches(batches, word2id, char2id, context_file, qn_file, ans_file, batch_size, context_len, question_len,char_len, discard_long)
        if len(batches) == 0:
            break

        # Get next batch. These are all lists length batch_size
        (context_ids, context_tokens, qn_ids, qn_tokens,char_context_ids, char_qn_ids,ans_span, ans_tokens) = batches.pop(0)
        ...
        #Converting char_context_ids and char_qn_ids into a numpy array
        for i,char_context in enumerate(char_context_ids):
            a = np.zeros([context_len,char_len])
            a[0:len(char_context),:] = char_context
            char_context_ids[i] = a

        char_context_ids = np.array(char_context_ids)

        for i,char_qn in enumerate(char_qn_ids):
            a = np.zeros([question_len,char_len])
            a[0:len(char_qn),:] = char_qn
            char_qn_ids[i] = a
        ...
        batch = Batch(context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens,char_context_ids, char_qn_ids,ans_span, ans_tokens)
