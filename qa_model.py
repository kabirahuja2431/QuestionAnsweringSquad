...
class QAModelCharCNN(QAModel):
	...
	def add_placeholders(self):
		...
		self.context_ids = tf.placeholder(tf.int32, shape=[None, self.FLAGS.context_len])
		self.context_mask = tf.placeholder(tf.int32, shape=[None, self.FLAGS.context_len])
		self.char_context_ids = tf.placeholder(tf.int32,shape = [None,self.FLAGS.context_len,self.FLAGS.char_len])

		self.qn_ids = tf.placeholder(tf.int32, shape=[None, self.FLAGS.question_len])
		self.qn_mask = tf.placeholder(tf.int32, shape=[None, self.FLAGS.question_len])
		self.ans_span = tf.placeholder(tf.int32, shape=[None, 2])
		self.char_qn_ids = tf.placeholder(tf.int32,shape = [None,self.FLAGS.question_len,self.FLAGS.char_len])
	...
	def add_embedding_layer(self, emb_matrix):
			...
			with vs.variable_scope("embeddings"):
				...
				# Note: the embedding matrix is a tf.constant which means it's not a trainable parameter
				embedding_matrix = tf.constant(emb_matrix, dtype=tf.float32, name="emb_matrix") # shape (400002, embedding_size)
				char_embedding_matrix = tf.get_variable("char_embedding_matrix",shape = [self.FLAGS.char_vocab_len,self.FLAGS.char_embedding_size], dtype = tf.float32)
				...
				self.context_embs = embedding_ops.embedding_lookup(embedding_matrix, self.context_ids) # shape (batch_size, context_len, embedding_size)
				self.qn_embs = embedding_ops.embedding_lookup(embedding_matrix, self.qn_ids) # shape (batch_size, question_len, embedding_size)
				self.char_context_embs = tf.nn.embedding_lookup(char_embedding_matrix,tf.reshape(self.char_context_ids,[-1,self.FLAGS.char_len]))
				self.char_qn_embs = tf.nn.embedding_lookup(char_embedding_matrix,tf.reshape(self.char_qn_ids,[-1,self.FLAGS.char_len]))
	...
	def get_dev_loss(self, session, dev_context_path, dev_qn_path, dev_ans_path):
		...
		for batch in data_batcher_chars.get_batch_generator(self.word2id, self.char2id, dev_context_path, dev_qn_path, dev_ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, char_len = self.FLAGS.char_len, discard_long=True):
	...
	def check_f1_em(self, session, context_path, qn_path, ans_path, dataset, num_samples=100, print_to_screen=False):
		...
		# Note here we select discard_long=False because we want to sample from the entire dataset
		# That means we're truncating, rather than discarding, examples with too-long context or questions
		for batch in data_batcher_chars.get_batch_generator(self.word2id, self.char2id, context_path, qn_path, ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, char_len = self.FLAGS.char_len, discard_long=True):
	...
	def train(self, session, train_context_path, train_qn_path, train_ans_path, dev_qn_path, dev_context_path, dev_ans_path):
		...
		while self.FLAGS.num_epochs == 0 or epoch < self.FLAGS.num_epochs:
			...
			# Loop over batches
			for batch in data_batcher_chars.get_batch_generator(self.word2id, self.char2id, train_context_path, train_qn_path, train_ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, char_len = self.FLAGS.char_len, discard_long=True):
